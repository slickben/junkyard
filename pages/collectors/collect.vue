<template>
  ...
  >
    <div class="md:w-[741px]">
      ...
      <NuxtLink to="/collectors" class="text-textGray text-sm font-semibold"
        >Dashboard >
      ...
      <form @submit.prevent="onSubmit" class="mt-5 md:mt-12">
        ...
        >
          <div class="col-span-2">
            ...
            <div class="w-full rounded-2xl md:bg-white px-4 md:px-6 py-5 md:py-6 border">
              <HeadDisclosure
              ...
              >
                ...
                <HeadDisclosurePanel
                  class="grid grid-cols-2 gap-x-3 px-4 pb-2 pt-4 text-sm text-gray-500 border-2 border-secondary rounded-b-lg space-y-2"
                >
          ...
          <button
            type="submit"
            class="border border-secondary text-white bg-secondary text-lg font-semibold px-3 py-1 mb-7 rounded-lg hover:bg-white hover:text-secondary hover:border-[3px] hover:border-secondary transition-all transform ease-in-out duration-300"
          >
            Buy
          </button>
        </div>
              ...
              <div>
                <div class="space-y-8">
                  ...
                  <div class="flex flex-col relative">
                    ...
                    />
                    <p
                      class="absolute inset-x-0 -bottom-6 text-sm text-red-500"
                    >
                      {{ errors["accountDetails.account_number"] }}
                    </p>
                  </div>
                  <div v-if="account_name" class="flex flex-col relative">
                    ...
                    <p
                      class="absolute inset-x-0 -bottom-6 text-sm text-red-500"
                    >
                      {{ errors["accountDetails.account_name"] }}
                    </p>
                  </div>
                </div>
                <div class="flex items-center justify-between gap-x-10 pt-6">
                  <button
                    type="button"
                    @click="verifyBank"
                    class="text-center flex-1 p-4 rounded-md text-white font-bold hover:opacity-80 transition-all duration-300 ease-in-out py-4 relative bg-secondary"
                  >
                    <svg
                      v-show="isLoading"
    ...
    yup.object({
      ...
      user: yup.object({
        ...
        phoneNumber: yup
          ...
          .required()
          .label("Phone number")
          .min(12, "Number must be 11 digit"),
...
const [account_name, account_nameAttrs] = defineField(
  "accountDetails.account_name"
);
const [account_number, account_numberAttrs] = defineField(
...
const getAddress = async (lat: number, long: number) => {
  await $fetch(
    `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${long}&key=AIzaSyB2O2LnwRIFd2OM1qXvIX8s25d7orAisdE`,
    {
      onResponse({ request, response, options }) {
        // Process the response data
        //   isLoading.value = false
        if (response.ok) {
          ...
          setFieldValue(
            "user.address",
            response._data.results[0].formatted_address
          );
          //   banks.value = response._data.data
          // payment_modal.value = true
          // overview.value = response._data
