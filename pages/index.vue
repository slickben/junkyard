<template>
  ...
  <div
    class="bg-fakeWhite h-full flex-grow p-5 md:p-12 2xl:px-20 pt-0 md:pt-10 relative"
  >
    <div class="mb-5 flex items-center justify-between">
      ...
      <div class="flex items-center gap-4">
        ...
        <button
          ...
          class="text-error text-lg font-semibold hidden md:block"
        >
          Logout
        </button>
      </div>
    ...
    <div
      class="flex justify-between text-sm 2xl:text-xl xl:text-lg m-7 mx-0 md:mx-7"
    >
    ...
    <div
      class="card-shadow rounded-xl 2xl:max-h-[620px] xl:max-h-[460px] overflow-y-auto hide-scroll hidden md:block"
    >
      <table class="w-full font-semibold 2xl:text-lg xl:text-sm table-auto">
        <thead
          class="text-textGray text-lg font-semibold sticky top-0 bg-white"
        >
        ...
        <tbody class="">
          <tr
            ...
            :key="item.id"
            class="border-b text-[16px] font-medium"
          >
    ...
    <div
      class="rounded-xl 2xl:max-h-[620px] xl:max-h-[460px] overflow-y-auto hide-scroll md:hidden space-y-2"
    >
      <div
        v-for="item in collections"
        :key="item.id"
        class="flex items-center bg-white px-3 py-2 space-x-1"
      >
        <vue-avatar
          class="flex-none"
          :username="item.collector_name"
          :size="40"
        />
...
const getTodaysCollection = async (limit: number, page: number) => {
  ...
  await $fetch(
    `${useBaseUrl()}/admin/collections/today?limit=${limit}&page=${page}`,
    {
      headers: {
        Authorization: `${token.value}`,
      },
