        ...
        <div class="grid 2xl:grid-cols-4 md:grid-cols-3 pb gap-10 px-5 md:px-12 2xl:px-20">
          ...
          <NuxtLink :to="`/super-admin/customers/collector/${item.id}`" v-for="item in collectors" :key="item.id"
           class="rounded-2xl border-[3px] border-secondary shadow-xl flex items-center md:flex-col p-4">
            ...
            >
              ...
              <p class=" truncate"><span class="hidden md:inline-block">Name:</span> {{item.name}}</p>
              <p><span class="hidden md:inline-block">Email:</span> {{item.email}}</p>
            </div>
          </NuxtLink>
          ...
          >
            <NuxtLink to="recyclers/create"
              ><button
                class="bg-secondary text-white rounded-md card-shadow 
                md:px-12 px-5 py-3 flex justify-center items-center text-sm md:text-xl gap-4 
                font-semibold
                 hover:opacity-45 transition-all transform ease-out duration-300"
              >
                <span class=""> <img class="h-4 md:h-7" src="/img/addUnit.svg" alt="" /> </span>ADD
                NEW UNIT
              </button>
            </NuxtLink>
          </div> -->
