<template>
  ...
  <div
    class="bg-[#F5F5F5] md:bg-fakeWhite h-full flex-grow p-5 md:p-12 2xl:px-20 pt-0 md:pt-10 relative overflow-y-auto"
  >
    ...
    <div
      class="rounded-xl 2xl:max-h-[620px] xl:max-h-[460px] overflow-y-auto hide-scroll md:hidden space-y-2 cursor-pointer"
    >
      <div
        v-for="item in collections"
        :key="item.id"
...
const getTodaysCollection = async (limit: number, page: number) => {
  ...
  await $fetch(
    `${useBaseUrl()}/super/collections?limit=${limit}&page=${page}`,
    {
      headers: {
        Authorization: `${token.value}`,
      },
