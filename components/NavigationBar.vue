...
<div class="flex items-center justify-between md:hidden p-4 sticky top-0 z-50 bg-[#F5F5F5] ">
        ...
        <Menu 
            ...
            >
                <MenuItems class="absolute w-60 right-0 bg-white p-5 space-y-3">
                    <MenuButton>
                        <div
                            class="flex gap-4 items-center row-span-1"
                        >
                            <div class="relative">
                            <img v-if="user.data.avatar" :src="user.data.avatar" alt="" class="w-[30px] h-[30px] rounded-full object-cover" />
                            <vue-avatar class="" v-else :username="`${user.data.role === 'collector' ? user.data.name : user.data.businessInfo.name}`" :size="30" />
                    ...
                    <MenuItem v-slot="{ active }">
                        <button type="button" @click.prevent="signOut({ callbackUrl: '/login' })" class="text-error text-lg font-semibold flex  items-center gap-2">
...
<script lang="ts" setup>
    ...
    import { XMarkIcon, Bars3Icon, ChevronDownIcon, ArrowLeftStartOnRectangleIcon } from "@heroicons/vue/24/outline";
